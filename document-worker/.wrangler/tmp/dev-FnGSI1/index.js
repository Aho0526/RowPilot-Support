...
// .wrangler/tmp/bundle-RZ7SvB/checked-fetch.js
...
// .wrangler/tmp/bundle-RZ7SvB/strip-cf-connecting-ip-header.js
...
var err = /* @__PURE__ */ __name((c, status, message) => c.json({ error: message }, status), "err");
app.get("/document/api/essays/:id", async (c) => {
  ...
  if (!essay) {
    ...
    essay = { id, title: "\u8AB2\u984C\u8AD6\u6587", current_content: "", updated_at: (/* @__PURE__ */ new Date()).toISOString() };
  }
...
app.patch("/document/api/essays/:id", async (c) => {
  ...
  if (typeof body.content !== "string")
    return err(c, 400, "content is required");
  const result = await db.prepare(
    `UPDATE essays SET current_content = ?, updated_at = datetime('now') WHERE id = ?`
  ).bind(body.content, id).run();
  if (result.meta.changes === 0)
    return err(c, 404, "Essay not found");
  return c.json({ success: true, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
});
...
app.get("/document/api/essays/:id/versions/:vid", async (c) => {
  ...
  if (!version)
    return err(c, 404, "Version not found");
...
app.get("/document/api/reviews", async (c) => {
  ...
  if (!versionId)
    return err(c, 400, "version_id is required");
...
app.patch("/document/api/reviews/:id", async (c) => {
  ...
  if (!review)
    return err(c, 404, "Review not found");
  if (review.submitted_at)
    return err(c, 403, "Review already submitted");
  let body;
  try {
    body = await c.req.json();
  } catch {
  ...
  if (typeof body.markdown_comment === "string") {
    stmts.push(
      db.prepare(`UPDATE reviews SET markdown_comment = ? WHERE id = ?`).bind(body.markdown_comment, reviewId)
    );
  }
...
app.post("/document/api/reviews/:id/submit", async (c) => {
  ...
  if (!review)
    return err(c, 404, "Review not found");
  if (review.submitted_at)
    return err(c, 400, "Already submitted");
  await db.prepare(
...
// .wrangler/tmp/bundle-RZ7SvB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RZ7SvB/middleware-loader.entry.ts
